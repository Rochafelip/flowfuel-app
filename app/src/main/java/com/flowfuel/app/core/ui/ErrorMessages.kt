...
fun AppError.userMessage(): String = when (this) {
    ...
    is AppError.Api -> when (code) {
        ...
        "AUTH_REFRESH_EXPIRED" -> stringResource(R.string.error_session_expired)
        "AUTH_REFRESH_REVOKED" -> stringResource(R.string.error_session_revoked)
        "AUTH_REFRESH_INVALID" -> stringResource(R.string.error_session_expired)
